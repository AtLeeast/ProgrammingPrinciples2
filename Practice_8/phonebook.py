...
def drop_table():
    command = "DROP TABLE contacts CASCADE"
...
def insert_from_csv():
     with conn.cursor() as cur:
        command = "INSERT INTO contacts(name, phone) VALUES(%s, %s)"
        with open("Practice_8/contacts.csv", "r") as f:
            reader = csv.reader(f)
...
with open("Practice_8/functions.sql", "r") as f:
    with conn.cursor() as cur:
        cur.execute(f.read())
    conn.commit()

with open("Practice_8/procedures.sql", "r") as d:
    with conn.cursor() as cur:
        cur.execute(d.read())
    conn.commit()
